...
get(["status"]) ->
  case rabint:call({rabbit, status, []}) of
    Bin -> 
	  ...
	  %Extracts the rabbitMQ version number
	  [H|_] = Bin,
	  {running_applications, [{_,_,NRabbitver}| _]} = H,
	  Rabbitver = erlang:list_to_binary(NRabbitver),
	  
	  Jsonable = [{struct, [{"rabbitver", Rabbitver},{"applications", Apps}, {"nodes", Nodes},
